...
class SOQUTE_CORESHARED_EXPORT DependencyCalculator : public QObject
{
    Q_OBJECT
public:
	explicit DependencyCalculator(PackageList* packageList, PackagePointerList packages, QObject *parent = 0);
     ...
     */
	PackagePointerList result() const { return m_result; }

public slots:
    /**
     * This function will calculate the dependencies of all packages specified in the constructor recursivly,
     * and add them to the result which can be accesses using \see result
     */
    bool calculateDependencies(const Direction direction = Forward, const Package **notFoundPackage = 0);
