...
DependencyCalculator::DependencyCalculator(PackageList *packageList,
										   PackagePointerList packages, QObject *parent)
	: QObject(parent), m_startingPoint(packages), m_packages(packageList)
{
}

bool DependencyCalculator::calculateDependencies(const Direction direction,
												 const Package **notFoundPackage)
{
	...
	if (direction == Forward) {
		...
		for (PackagePointer package : m_startingPoint) {
			...
			PackagePointerList res =
				calculateDependenciesForward(package, &ok, notFoundPackage);
			if (!ok) {
				return false;
			}
			result.append(res);
...
PackagePointerList
	DependencyCalculator::calculateDependenciesForward(const Package *package, bool *ok,
													   const Package **notFoundPackage)
{
...
PackagePointerList DependencyCalculator::calculateDependenciesReverse(const Package *package,
