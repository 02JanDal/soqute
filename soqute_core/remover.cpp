...
Remover::Remover(QObject *parent) :
    QObject(parent), m_remover(new JSInstaller(this))
{
    connect(m_remover, SIGNAL(error(QString)), this, SLOT(errorRemoving(QString)));
    connect(m_remover, SIGNAL(removePackageBegin(const Package*)), this, SLOT(removePackageBegin(const Package*)));
    connect(m_remover, SIGNAL(removePackageEnd(const Package*)), this, SLOT(removePackageEnd(const Package*)));
    connect(m_remover, SIGNAL(finished()), this, SLOT(finish()));
    connect(this, SIGNAL(removePackage(const Package*,QString)), m_remover, SLOT(remove(const Package*,QString)), Qt::QueuedConnection);
}
...
void Remover::removePackageEnd(const Package *package)
{
    ...
    ConfigurationHandler::instance()->installedPackages()->setPackageUninstalled(package->id(), package->version(), package->platform());
...
void Remover::removeNextPackage()
{
    ...
    emit removePackage(package, Util::removalScriptsDirectory().absoluteFilePath(QString("%1-%2-%3-remove.js").arg(package->id(), package->version(), package->platform())));
}
